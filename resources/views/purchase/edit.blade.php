...
@section('content')
    ...
    <div class="row" style="min-height:195px" onkeypress="formEnterClick(event)">
    	<div class="col-md-4" id="col_1">
            ...
            {!! Former::select('client')
					...
					->addGroupClass('client_select closer-row')->label('Supplier') !!}
			<div style="display: none">
				{!! Former::text('client_id')
					->hidden()
					->addClass('client-input-id') !!}
			</div>
		...
		<div class="col-md-4" id="col_2">
            ...
            @if ($entityType == ENTITY_PURCHASE_ORDER)
		...
		<div class="col-md-4" id="col_2">
            ...
            {!! Former::text('invoice_number')
                        ...
                        ->addGroupClass('invoice-number')->label('Purchase Order')
			...
			{!! Former::text('po_number')->label($account->getLabel('po_number', 'po_number_short'))->label('Quote')->data_bind("value: po_number, valueUpdate: 'afterkeydown'") !!}
	...
	<div class="table-responsive" style="padding-top:4px;">
		...
		@include('purchase.edit_table', ['isTasks' => false])
		...
		<table class="pull-right subtotals-table" style="margin-right:40px; margin-top:0px;">
			...
			<tr data-bind="style: { 'font-weight': partial() ? 'normal' : 'bold', 'font-size': partial() ? '1em' : '1.05em' }" style="font-size:1.05em;font-weight:bold;">
				<td class="hide-border" data-bind="css: {'hide-border': !partial()}" colspan="2">Balance to be paid</td>
	...
	@include('purchase.pdf', ['account' => Auth::user()->account, 'hide_pdf' => ! Auth::user()->account->live_preview])
        ...
        @else
            ...
            @if (isset($expenses) && $expenses->count())
                ...
                for (var i=0; i<expenses.length; i++) {
                    ...
                    item.raw_material_key(expense.expense_category ? expense.expense_category.name : '');
			...
			@if ($selectedProducts = session('selectedProducts'))
				...
				for (var i=0; i<products.length; i++) {
					...
					productMap[product.raw_material_key] = product;
				}
