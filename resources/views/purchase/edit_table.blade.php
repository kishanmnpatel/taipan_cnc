    ...
    <tr data-bind="event: { mouseover: showActions, mouseout: hideActions }" class="sortable-row">
        ...
        <td>
            <div id="scrollable-dropdown-menu">
                <input type="text" data-bind="productTypeahead: raw_material_key, items: $root.products, key: 'raw_material_key', valueUpdate: 'afterkeydown', attr: {name: 'invoice_items[{{ $isTasks ? 'T' : '' }}' + $index() + '][raw_material_key]'}" class="form-control invoice-item handled"/>
            </div>
        </td>
        <td>
            <textarea data-bind="value: notes, valueUpdate: 'afterkeydown', attr: {name: 'invoice_items[{{ $isTasks ? 'T' : '' }}' + $index() + '][notes]'}"
                ...
                <input type="text" data-bind="value: public_id, attr: {name: 'invoice_items[{{ $isTasks ? 'T' : '' }}' + $index() + '][public_id]'}" style="display: none"/>
        </td>
        @if ($account->customLabel('product1'))
            <td>
