...
function ViewModel(data) {
    ...
    self.clientLinkText = ko.computed(function() {
        ...
        {
            return "{{ trans('texts.edit_supplier') }}";
        }
        else
        {
            ...
            {
                return "{{ trans('texts.create_new_supplier') }}";
            }
        }
    });
...
function ItemModel(data) {
    var self = this;
    self.raw_material_key = ko.observable('');
    ...
    this.isEmpty = function() {
        return !self.raw_material_key() && !self.notes() && !self.cost( )&& !self.public_id();
    }
            ...
            @if (Auth::user()->account->show_product_notes)
                templates: {
                    suggestion: function(item) { return '<div title="' + _.escape(item.notes) + '" style="border-bottom: solid 1px #CCC">'
                        + _.escape(item.raw_material_key) + "<br/>"
